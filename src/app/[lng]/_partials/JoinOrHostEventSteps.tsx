...
import {
  hostEventSteps,
  joinEventSteps,
} from "@/app/[lng]/_partials/homePageData";
...
export default function JoinOrHostEventSteps({ lng }: { lng: string }) {
  const [stepType, setStepType] = useState<"join" | "host">("join");
  const { clientWidth } = useResponsiveSizes();
  const { t } = useTranslation(lng);

  const steps = t(
    stepType === "join" ? "join.joinEventSteps" : "join.hostEventSteps",
    {
      returnObjects: true,
    }
  ) as Array<{ title: string; desc: string; alt: string }>;
  return (
    <>
        ...
        {steps.map((step, i) => (
          ...
          >
            <span className="absolute right-0 top-0 -translate-y-1/2">
              {
                (stepType === "join" ? joinEventSteps : hostEventSteps)[i]
                  .stepNum
              }
            </span>
            ...
            <div className="bg-white flex justify-center items-center rounded-[10px] h-[177px] overflow-hidden">
              <Image
                src={
                  (stepType === "join" ? joinEventSteps : hostEventSteps)[i].img
                }
