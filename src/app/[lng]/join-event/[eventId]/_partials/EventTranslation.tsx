...
import RejoinEventModal from "@/app/[lng]/dashboard/liveTranslation/_partials/RejoinEventModal";
...
export default function EventTranslation({
  ...
  useEffect(() => {
    if (
      event?.eventIsOngoing &&
      !hasTriedJoinRef.current &&
      hasCompletedTourVisitor
    ) {
      setTranslationLanguage({
        value: "EN_GB",
  ...
  }, [event?.eventIsOngoing, joinEvent, hasCompletedTourVisitor]);
